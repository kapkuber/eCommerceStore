...
export default function AccountTopNav({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  if (isAdmin) items.push({ label: "INVENTORY DASHBOARD", href: "#product-dashboard" });
  ...
  return (
    <nav className="sticky top-0 z-30 w-full border-b bg-[#525252] py-7 text-white">
      <ul className="mx-auto flex max-w-5xl items-center justify-between gap-6 px-6 text-xs font-bold tracking-wide">
        {items.map((it) => (
          <li key={it.label}>
            {it.href ? (
